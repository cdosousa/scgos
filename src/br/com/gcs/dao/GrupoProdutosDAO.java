 ...
 */
public class GrupoProdutosDAO extends AbstractTableModel {
    ...
    public GrupoProdutosDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(GrupoProdutos gp) {
        String sql = "INSERT INTO GCSGRUPOS(CD_GRUPO, NOME_GRUPO, GERAR_CODIGO, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, gp.getCdGrupo());
            pstmt.setString(2, gp.getNomeGrupo());
            pstmt.setString(3, gp.getGerarCodigo());
            pstmt.setString(4, gp.getUsuarioCadastro());
            pstmt.setString(5, gp.getDataCadastro());
            pstmt.setString(6, gp.getDataModificacao());
            pstmt.setString(7, gp.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Grupo já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação do Grupo, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(GrupoProdutos gp){
        String sql = "UPDATE GCSGRUPOS SET NOME_GRUPO = ?,"
                + "GERAR_CODIGO = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_GRUPO = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, gp.getNomeGrupo());
            pstmt.setString(2, gp.getGerarCodigo());
            pstmt.setString(3, gp.getDataModificacao());
            pstmt.setString(4, gp.getSituacao());
            pstmt.setString(5, gp.getCdGrupo());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<GrupoProdutos> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new GrupoProdutos(
    ...
    public void excluir(GrupoProdutos cc) {
        String sql = "DELETE FROM GCSGRUPOS WHERE CD_GRUPO = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
