 ...
 */
public class UnidadesMedidaDAO extends AbstractTableModel {
    ...
    public UnidadesMedidaDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(UnidadesMedida um) {
        String sql = "INSERT INTO GCSUNIDMEDIDA(CD_UNIDMEDIDA, NOME_UNIDMEDIDA, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, um.getCdUnidMedida());
            pstmt.setString(2, um.getNomeUnidMedida());
            pstmt.setString(3, um.getUsuarioCadastro());
            pstmt.setString(4, um.getDataCadastro());
            pstmt.setString(5, um.getDataModificacao());
            pstmt.setString(6, um.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Unidade de Medida já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação da Unidade de Medida, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(UnidadesMedida um){
        String sql = "UPDATE GCSUNIDMEDIDA SET NOME_UNIDMEDIDA = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_UNIDMEDIDA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, um.getNomeUnidMedida());
            pstmt.setString(2, um.getDataModificacao());
            pstmt.setString(3, um.getSituacao());
            pstmt.setString(4, um.getCdUnidMedida());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<UnidadesMedida> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new UnidadesMedida(
    ...
    public void excluir(UnidadesMedida cc) {
        String sql = "DELETE FROM GCSUNIDMEDIDA WHERE CD_UNIDMEDIDA = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getCdUnidMedida());
            pstmt.execute();
            pstmt.close();
