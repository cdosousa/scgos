 ...
 */
public class EssenciaProdutosDAO extends AbstractTableModel {
    ...
    public EssenciaProdutosDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(EssenciaProdutos ep) {
        String sql = "INSERT INTO GCSESSENCIA(CD_ESSENCIA, NOME_ESSENCIA, GERAR_CODIGO, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ep.getCdEssencia());
            pstmt.setString(2, ep.getNomeEssencia());
            pstmt.setString(3, ep.getGerarCodigo());
            pstmt.setString(4, ep.getUsuarioCadastro());
            pstmt.setString(5, ep.getDataCadastro());
            pstmt.setString(6, ep.getDataModificacao());
            pstmt.setString(7, ep.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Essencia já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação da Essencia, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(EssenciaProdutos ep){
        String sql = "UPDATE GCSESSENCIA SET NOME_ESSENCIA = ?,"
                + "GERAR_CODIGO = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_ESSENCIA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ep.getNomeEssencia());
            pstmt.setString(2, ep.getGerarCodigo());
            pstmt.setString(3, ep.getDataModificacao());
            pstmt.setString(4, ep.getSituacao());
            pstmt.setString(5, ep.getCdEssencia());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<EssenciaProdutos> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new EssenciaProdutos(
                                rs.getString("cd_essencia"),
                                rs.getString("nome_essencia"),
    ...
    public void excluir(EssenciaProdutos cc) {
        String sql = "DELETE FROM GCSESSENCIA WHERE CD_ESSENCIA = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getCdEssencia());
            pstmt.execute();
            pstmt.close();
