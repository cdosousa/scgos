 ...
 */
public class MarcaProdutosDAO extends AbstractTableModel {
    ...
    public MarcaProdutosDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(MarcaProdutos mp) {
        String sql = "INSERT INTO GCSMARCA(CD_MARCA, NOME_MARCA, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, mp.getCdMarca());
            pstmt.setString(2, mp.getNomeMarca());
            pstmt.setString(3, mp.getUsuarioCadastro());
            pstmt.setString(4, mp.getDataCadastro());
            pstmt.setString(5, mp.getDataModificacao());
            pstmt.setString(6, mp.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Marca já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação da Marca, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(MarcaProdutos mp){
        String sql = "UPDATE GCSMARCA SET NOME_MARCA = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_MARCA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, mp.getNomeMarca());
            pstmt.setString(2, mp.getDataModificacao());
            pstmt.setString(3, mp.getSituacao());
            pstmt.setString(4, mp.getCdMarca());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<MarcaProdutos> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new MarcaProdutos(
    ...
    public void excluir(MarcaProdutos cc) {
        String sql = "DELETE FROM GCSMARCA WHERE CD_MARCA = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getCdMarca());
            pstmt.execute();
            pstmt.close();
