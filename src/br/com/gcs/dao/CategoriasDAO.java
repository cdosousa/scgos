 ...
 */
public class CategoriasDAO extends AbstractTableModel {
    ...
    public CategoriasDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(Categorias cat) {
        String sql = "INSERT INTO GCSCATEGORIA(CD_CATEGORIA, NOME_CATEGORIA, GERAR_CODIGO, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cat.getCdCategoria());
            pstmt.setString(2, cat.getNomeCategoria());
            pstmt.setString(3, cat.getGerarCodigo());
            pstmt.setString(4, cat.getUsuarioCadastro());
            pstmt.setString(5, cat.getDataCadastro());
            pstmt.setString(6, cat.getDataModificacao());
            pstmt.setString(7, cat.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Categoria já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação da Categoria, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    ...
    public void atualizar(Categorias cat){
        String sql = "UPDATE GCSCATEGORIA SET NOME_CATEGORIA = ?,"
                + "GERAR_CODIGO = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_CATEGORIA = ?";
        try{
            ...
            pstmt.setString(2, cat.getGerarCodigo());
            pstmt.setString(3, cat.getDataModificacao());
            pstmt.setString(4, cat.getSituacao());
            pstmt.setString(5, cat.getCdCategoria());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<Categorias> resultado, String sql) {
        try {
            if (!statusConexao) {
    ...
    public void excluir(Categorias cc) {
        String sql = "DELETE FROM GCSCATEGORIA WHERE CD_CATEGORIA = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getCdCategoria());
            pstmt.execute();
            pstmt.close();
