 ...
 */
public class ArmazensDAO extends AbstractTableModel {
    ...
    public ArmazensDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(Armazens cd) {
        String sql = "INSERT INTO GCSARMAZEM(CD_ARMAZEM, NOME_ARMAZEM, TIPO_ARMAZEM, PERMITE_SEPARACAO, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cd.getCdArmazem());
            pstmt.setString(2, cd.getNomeArmazem());
            pstmt.setString(3, cd.getTipoArmazem());
            pstmt.setString(4, cd.getPermiteSeparacao());
            pstmt.setString(5, cd.getUsuarioCadastro());
            pstmt.setString(6, cd.getDataCadastro());
            pstmt.setString(7, cd.getDataModificacao());
            pstmt.setString(8, cd.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Armazém já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação do Armazém, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(Armazens cd){
        String sql = "UPDATE GCSARMAZEM SET NOME_ARMAZEM = ?,"
                + "TIPO_ARMAZEM = ?,"
                + "PERMITE_SEPARACAO = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
        ...
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cd.getNomeArmazem());
            pstmt.setString(2, cd.getTipoArmazem());
            pstmt.setString(3, cd.getPermiteSeparacao());
            pstmt.setString(5, cd.getDataModificacao());
            pstmt.setString(5, cd.getSituacao());
            pstmt.setString(6, cd.getCdArmazem());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<Armazens> resultado, String sql) {
        try {
            if (!statusConexao) {
    ...
    public void excluir(Armazens cc) {
        String sql = "DELETE FROM GCSARMAZEM WHERE CD_ARMAZEM = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getCdArmazem());
            pstmt.execute();
            pstmt.close();
