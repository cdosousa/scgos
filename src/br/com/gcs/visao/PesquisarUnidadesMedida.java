 ...
 */
public class PesquisarUnidadesMedida extends JDialog{
    //Variáveis de instância privadas
    private UnidadesMedidaDAO umDAO;
    ...
    //Contrutor padrão
    public PesquisarUnidadesMedida(Frame parent, boolean modal, String chamador, Connection conexao){
        super(parent, modal);
    ...
    public void initComponentes(){
        ...
        tabelaPes = new JTable(umDAO);
                        
        //Definindo o tamanho da Janela
        AjustarTamanhoTabela();
        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setModal(true);
        setTitle("Pesquisar Unidades de Medida");
    ...
    public void buscarTodos(){
        try{
            umDAO = new UnidadesMedidaDAO(conexao);
            umDAO.setQuery("SELECT CD_UNIDMEDIDA AS Código,"
                    + "NOME_UNIDMEDIDA AS Nome,"
    ...
    public void buscarPorNome(){
        try{
            String query = "SELECT CD_UNIDMEDIDA AS Código,"
                    + "NOME_UNIDMEDIDA AS Nome,"
                    + "DATA_CADASTRO AS Cadastro,"
                    + "DATA_MODIFICACAO AS Modificao"
                    + ",SITUACAO AS Sit"
                    + " FROM GCSUNIDMEDIDA WHERE NOME_UNIDMEDIDA LIKE "
                    + "'%"
                    + jTexPesqNome.getText()
                    + "%'";
            umDAO.setQuery(query);
    ...
    public void AjustarTamanhoTabela(){
        ...
        tabelaPes.getColumnModel().getColumn(1).setMinWidth(400);
        tabelaPes.getColumnModel().getColumn(1).setMaxWidth(400);
        tabelaPes.getColumnModel().getColumn(2).setMinWidth(75);
        tabelaPes.getColumnModel().getColumn(2).setMaxWidth(75);
        tabelaPes.getColumnModel().getColumn(3).setMinWidth(75);
        tabelaPes.getColumnModel().getColumn(3).setMaxWidth(75);
        tabelaPes.getColumnModel().getColumn(4).setMinWidth(30);
        tabelaPes.getColumnModel().getColumn(4).setMaxWidth(30);
        if (indexAtual < 0) {
            indexAtual = 0;
        }
    ...
    public static void main(String[]args){
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {                
                PesquisarUnidadesMedida dialog = new PesquisarUnidadesMedida(new JFrame(),true,chamador, conexao);
