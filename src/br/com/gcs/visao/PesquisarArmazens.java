 ...
 */
public class PesquisarArmazens extends JDialog{
    //Variáveis de instância privadas
    private ArmazensDAO cdDAO;
    ...
    //Contrutor padrão
    public PesquisarArmazens(Frame parent, boolean modal, String chamador,Connection conexao){
        super(parent, modal);
    ...
    public void initComponentes(){
        ...
        tabelaPes = new JTable(cdDAO);
                        
        //Definindo o tamanho da Janela
        AjustarTamanhoTabela();
        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setModal(true);
        setTitle("Pesquisar Armazéns");
    ...
    public void buscarTodos(){
        try{
            ...
            cdDAO.setQuery("SELECT CD_ARMAZEM AS Código,"
                    + "NOME_ARMAZEM AS Nome,"
                    + "TIPO_ARMAZEM AS Tipo,"
                    + "PERMITE_SEPARACAO AS Separação,"
    ...
    public void buscarPorNome(){
        try{
            String query = "SELECT CD_ARMAZEM AS Código,"
                    + "NOME_ARMAZEM AS Nome,"
                    + "TIPO_ARMAZEM AS Tipo,"
                    + "PERMITE_SEPARACAO AS Separação,"
                    + "DATA_CADASTRO AS Cadastro,"
                    + "DATA_MODIFICACAO AS Modificao"
                    + ",SITUACAO AS Sit"
                    + " FROM GCSARMAZEM WHERE NOME_ARMAZEM LIKE "
                    + "'%"
                    + jTexPesqNome.getText()
                    + "%'";
            cdDAO.setQuery(query);
    ...
    public void AjustarTamanhoTabela(){
        ...
        tabelaPes.getColumnModel().getColumn(1).setMinWidth(250);
        tabelaPes.getColumnModel().getColumn(1).setMaxWidth(250);
        tabelaPes.getColumnModel().getColumn(2).setMinWidth(75);
        tabelaPes.getColumnModel().getColumn(2).setMaxWidth(75);
        tabelaPes.getColumnModel().getColumn(3).setMinWidth(80);
        tabelaPes.getColumnModel().getColumn(3).setMaxWidth(80);
    ...
    public static void main(String[]args){
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {                
                PesquisarArmazens dialog = new PesquisarArmazens(new JFrame(),true,chamador, conexao);
