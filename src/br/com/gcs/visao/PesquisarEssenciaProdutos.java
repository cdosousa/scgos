 ...
 */
public class PesquisarEssenciaProdutos extends JDialog{
    //Variáveis de instância privadas
    private EssenciaProdutosDAO epDAO;
    ...
    //Contrutor padrão
    public PesquisarEssenciaProdutos(Frame parent, boolean modal, String chamador, Connection conexao){
        super(parent, modal);
    ...
    public void initComponentes(){
        ...
        tabelaPes = new JTable(epDAO);
                        
        //Definindo o tamanho da Janela
        AjustarTamanhoTabela();
        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setModal(true);
        setTitle("Pesquisar Essencia de Produtos");
    ...
    public void buscarTodos(){
        try{
            epDAO = new EssenciaProdutosDAO(conexao);
            epDAO.setQuery("SELECT CD_ESSENCIA AS Código,"
                    + "NOME_ESSENCIA AS Nome,"
                    + "GERAR_CODIGO AS Forma_Codigo,"
    ...
    public void buscarPorNome(){
        try{
            String query = "SELECT CD_ESSENCIA AS Código,"
                    + "NOME_ESSENCIA AS Nome,"
                    + "GERAR_CODIGO AS Forma_Codigo,"
                    + "DATA_CADASTRO AS Cadastro,"
                    + "DATA_MODIFICACAO AS Modificao"
                    + ",SITUACAO AS Sit"
                    + " FROM GCSESSENCIA WHERE NOME_ESSENCIA LIKE "
                    + "'%"
                    + jTexPesqNome.getText()
                    + "%'";
            epDAO.setQuery(query);
    ...
    public void AjustarTamanhoTabela(){
        ...
        tabelaPes.getColumnModel().getColumn(2).setMinWidth(100);
        tabelaPes.getColumnModel().getColumn(2).setMaxWidth(100);
    ...
    public static void main(String[]args){
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {                
                PesquisarEssenciaProdutos dialog = new PesquisarEssenciaProdutos(new JFrame(),true,chamador, conexao);
