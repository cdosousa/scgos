 ...
 */
public class ItemTipoVernizDAO extends AbstractTableModel {
    ...
    public ItemTipoVernizDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(ItemTipoVerniz itv ) {
        String sql = "INSERT INTO GCVITEMTIPOVERNIZ(CD_TIPOVERNIZ, CD_MATERIAL, CD_UNIDMEDIDA, QTDE,"
                + " USUARIO_CADASTRO, DATA_CADASTRO, USUARIO_MODIFICACAO, DATA_MODIFICACAO, SITUACAO)"
                + " VALUES(?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, itv.getCdTipoVerniz());
            pstmt.setString(2, itv.getCdMaterial());
            pstmt.setString(3, itv.getCdUnidMedida());
            pstmt.setDouble(4, itv.getQtde());
            pstmt.setString(5, itv.getUsuarioCadastro());
            pstmt.setString(6, itv.getDataCadastro());
            pstmt.setString(7, itv.getUsuarioModificacao());
            pstmt.setString(8, itv.getDataModificacao());
            pstmt.setString(9, itv.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Item Tipo de Verniz já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação do Item Tipo de Verniz, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(ItemTipoVerniz itv){
        String sql = "UPDATE GCVITEMTIPOVERNIZ SET CD_UNIDMEDIDA = ?,"
                + " QTDE = ?,"
                + " USUARIO_MODIFICACAO = ? ,"
                + " DATA_MODIFICACAO = ?,"
                + " SITUACAO = ?"
                + " WHERE CD_TIPOVERNIZ = ? AND CD_MATERIAL = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, itv.getCdUnidMedida());
            pstmt.setDouble(2, itv.getQtde());
            pstmt.setString(3, itv.getUsuarioModificacao());
            pstmt.setString(4, itv.getDataModificacao());
            pstmt.setString(5, itv.getSituacao());
            pstmt.setString(6, itv.getCdTipoVerniz());
            pstmt.setString(7, itv.getCdMaterial());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(ItemTipoVerniz itv){
        String sql = "DELETE FROM GCVITEMTIPOVERNIZ WHERE CD_TIPOVERNIZ = ? AND CD_MATERIAL = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, itv.getCdTipoVerniz());
            pstmt.setString(2, itv.getCdMaterial());
    ...
    public void selecionar(List<ItemTipoVerniz> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new ItemTipoVerniz(
