 ...
 */
public class OperacaoVendaDAO extends AbstractTableModel {
    ...
    public OperacaoVendaDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(OperacaoVenda ove ) {
        String sql = "INSERT INTO GCVOPERVENDA(CD_OPERACAO, NOME_OPERACAO, CD_TIPOOPER, TIPO_FINALIDADE,"
                + "EMITE_NFE_VENDA, EMITE_NFE_SERVICO,GERA_COBRANCA, CD_CONTRATO, CD_CCUSTO, USUARIO_CADASTRO, DATA_CADASTRO,"
                + " DATA_MODIFICACAO, SITUACAO)"
                + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            String cdTipoOper = ove.getCdTipoOper();
            cdTipoOper = cdTipoOper.replace(".", "");
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ove.getCdOperacaoVenda());
            pstmt.setString(2, ove.getNomeOperacaoVenda());
            pstmt.setString(3, cdTipoOper);
            pstmt.setString(4, ove.getTipoFinalidade());
            pstmt.setString(5, ove.getEmiteNfeVenda());
            pstmt.setString(6, ove.getEmiteNfeServico());
            pstmt.setString(7, ove.getGeraCobranca());
            pstmt.setString(8, ove.getCdContrato());
            pstmt.setString(9, ove.getCdCCusto());
            pstmt.setString(10, ove.getUsuarioCadastro());
            pstmt.setString(11, ove.getDataCadastro());
            pstmt.setString(12, ove.getDataModificacao());
            pstmt.setString(13, ove.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Operação de Venda já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação da Operação de Venda, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(OperacaoVenda ove){
        String sql = "UPDATE GCVOPERVENDA SET NOME_OPERACAO = ?,"
                + " CD_TIPOOPER = ?,"
                + " TIPO_FINALIDADE = ?,"
                + " EMITE_NFE_VENDA = ?,"
                + " EMITE_NFE_SERVICO = ?,"
        ...
        try{
            ...
            cdTipoOper = cdTipoOper.replace(".", "");
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ove.getNomeOperacaoVenda());
            pstmt.setString(2, cdTipoOper);
            pstmt.setString(3, ove.getTipoFinalidade());
            pstmt.setString(4, ove.getEmiteNfeVenda());
            pstmt.setString(5, ove.getEmiteNfeServico());
            pstmt.setString(6, ove.getGeraCobranca());
            pstmt.setString(7, ove.getCdContrato());
            pstmt.setString(8, ove.getCdCCusto());
            pstmt.setString(9, ove.getDataModificacao());
            pstmt.setString(10, ove.getSituacao());
            pstmt.setString(11, ove.getCdOperacaoVenda());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(OperacaoVenda ove){
        String sql = "DELETE FROM GCVOPERVENDA WHERE CD_OPERACAO = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ove.getCdOperacaoVenda());
            pstmt.execute();
    ...
    public void selecionar(List<OperacaoVenda> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new OperacaoVenda(
                                rs.getString("cd_operacao"),
                                rs.getString("nome_operacao"),
                                rs.getString("cd_tipooper"),
                                rs.getString("tipo_finalidade"),
                                rs.getString("emite_nfe_venda"),
                                rs.getString("emite_nfe_servico"),
                                rs.getString("gera_cobranca"),
