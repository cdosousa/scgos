 ...
 */
public class ItemTabelaDAO extends AbstractTableModel {
    ...
    public ItemTabelaDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(ItemTabela ita ) {
        String sql = "INSERT INTO GCVITEMTABELA(CD_TABELA, CD_MATERIAL, CD_UNIDMEDIDA, VALOR_UNIT,"
                + " DESC_ALCADA, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO)"
                + " VALUES(?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ita.getCdTabela());
            pstmt.setString(2, ita.getCdMaterial());
            pstmt.setString(3, ita.getCdUnidMedida());
            pstmt.setDouble(4, ita.getValorUnit());
            pstmt.setDouble(5, ita.getDescAlcada());
            pstmt.setString(6, ita.getUsuarioCadastro());
            pstmt.setString(7, ita.getDataCadastro());
            pstmt.setString(8, ita.getDataModificacao());
            pstmt.setString(9, ita.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Item de Tabela já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação do Item da Tabela, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(ItemTabela ita){
        String sql = "UPDATE GCVITEMTABELA SET VALOR_UNIT = ?,"
                + " DESC_ALCADA = ?,"
                + " DATA_MODIFICACAO = ?,"
                + " SITUACAO = ?"
                + " WHERE CD_TABELA = ? AND CD_MATERIAL = ? AND CD_UNIDMEDIDA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setDouble(1, ita.getValorUnit());
            pstmt.setDouble(2, ita.getDescAlcada());
            pstmt.setString(3, ita.getDataModificacao());
            pstmt.setString(4, ita.getSituacao());
            pstmt.setString(5, ita.getCdTabela());
            pstmt.setString(6, ita.getCdMaterial());
            pstmt.setString(7, ita.getCdUnidMedida());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(ItemTabela ita){
        String sql = "DELETE FROM GCVITEMTABELA WHERE CD_TABELA = ?"
                + "AND CD_MATERIAL = ? AND CD_UNIDMEDIDA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ita.getCdTabela());
            pstmt.setString(2, ita.getCdMaterial());
    ...
    public void selecionar(List<ItemTabela> resultado, String sql) {
        try {
            if (!statusConexao) {
