 ...
 */
public class ManterClientesOld extends javax.swing.JFrame {
    ...
    /**
     * Contrutor padrão para iniciar a tela com um cliente fixo
     * @param su Sessao Atual do Usuario
     * @param conexao Objeto de conexão aberta com o bando de dados
     * @param sql Sentença SQL para buscar o cliente no banco de dados
     * @param isBotao Estado dos botões Novo, Editar, Salvar, Cancelar, Exculir e Salvar
     */
    public ManterClientesOld(SessaoUsuario su, Connection conexao, String sql, boolean isBotao) {
        this.su = su;
    ...
    public ManterClientesOld(SessaoUsuario su, Connection conexao) {
        this.su = su;
        this.conexao = conexao;
        oper = "N";
        initComponents();
    ...
    private void limparTela() {
        ...
        jComOptanteSimbahia.setSelectedIndex(0);
        jComOptanteSuframa.setSelectedIndex(0);
        jTexCdSuframa.setText("");
    ...
    private void upRegistros() {
        ...
        jComTipoPessoa.setSelectedIndex(Integer.parseInt(modcli.getTipoPessoa()));
        ...
        jComOptanteSimples.setSelectedIndex(Integer.parseInt(modcli.getOptanteSimples()));
        jComOptanteSimbahia.setSelectedIndex(Integer.parseInt(modcli.getOptanteSimbahia()));
        jComOptanteSuframa.setSelectedIndex(Integer.parseInt(modcli.getOptanteSuframa()));
        jTexCdSuframa.setText(modcli.getCdSuframa());
        ...
        jTexNumBanco.setText(modcli.getNumBanco());
        jTexNomeBanco.setText(modcli.getNomeBanco());
        jTexAgenciaBanco.setText(modcli.getAgenciaBanco());
        jTexNumContaBanco.setText(modcli.getNumContaBanco());
        jTexCdPortador.setText(modcli.getCdPortador());
        jTexNomePortador.setText(modcli.getNomePortador());
        jTexCdTipoPagamento.setText(modcli.getCdTipoPagamento());
        jTexNomeTipoPagamento.setText(modcli.getNomeTipoPagamento());
        jTexCdCondPag.setText(modcli.getCdCondPagamento());
        jTexNomeCondPag.setText(modcli.getNomeCondPag());
        jTexNomeContato.setText(modcli.getNomeContato());
        jForCelular.setText(modcli.getCelular());
        jForTelefone.setText(modcli.getTelefone());
        jTexEmail.setText(modcli.getEmail());
    ...
    private void bloquearCampos() {
        ...
        jComOptanteSimbahia.setEditable(false);
        jComOptanteSuframa.setEditable(false);
        jTexCdSuframa.setEditable(false);
    ...
    private void liberarCampos() {
        ...
        jComOptanteSimbahia.setEditable(true);
        jComOptanteSuframa.setEditable(true);
        jTexCdSuframa.setEditable(true);
...
// metodo para buscar CEP
    ...
    private void initComponents() {
        ...
        jLabReg.setText("\\");
            ...
            jTexSiglaUF.addCaretListener(new javax.swing.event.CaretListener() {
                public void caretUpdate(javax.swing.event.CaretEvent evt) {
                    jTexSiglaUFCaretUpdate(evt);
                }
            });
            ...
            jLabel2.setText("Optante Simbahia:");
    ...
    private void jButSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButSalvarActionPerformed
        ...
        if (jForCdCpfCnpj.getText().isEmpty() || jTexNomeRazaoSocial.getText().isEmpty() || jComSituacao.getSelectedItem().toString().substring(0, 1) == " "
            ...
            cpfCnpj = cpfCnpj.replace("-", "");
            cpfCnpj = cpfCnpj.replace("/", "");
            cli.setCdCpfCnpj(cpfCnpj);
            cli.setCdInscEstadual(jForInscEstadual.getText().trim().toUpperCase());
            cli.setTipoPessoa(jComTipoPessoa.getSelectedItem().toString().substring(0, 1));
            cli.setNomeRazaoSocial(jTexNomeRazaoSocial.getText().trim().toUpperCase());
            cli.setApelido(jTexApelido.getText().trim().toUpperCase());
            cli.setOptanteSimples(jComOptanteSimples.getSelectedItem().toString().substring(0, 1));
            cli.setOptanteSimbahia(jComOptanteSimbahia.getSelectedItem().toString().substring(0, 1));
            cli.setOptanteSuframa(jComOptanteSuframa.getSelectedItem().toString().substring(0, 1));
            cli.setCdSuframa(jTexCdSuframa.getText());
            cli.setTipoLogradouro(jComTipoLogradouro.getSelectedItem().toString().trim());
            cli.setLogradouro(jTexLogradouro.getText().trim().toUpperCase());
            cli.setNumero(jTexNumero.getText().trim().toUpperCase());
            cli.setComplemento(jTexComplemento.getText().trim().toUpperCase());
            cli.setBairro(jTexBairro.getText().trim().toUpperCase());
            cli.setCdMunicipioIbge(jTexCdMunicipioIbge.getText().trim());
            cli.setSiglaUf(jTexSiglaUF.getText().toUpperCase().toString().substring(jTexSiglaUF.getText().trim().length() - 2, 2));
            cli.setCdCep(jForCep.getText().toString().trim());
            cli.setNumBanco(jTexNumBanco.getText().trim().toUpperCase());
            cli.setNomeBanco(jTexNomeBanco.getText().trim().toUpperCase());
            cli.setAgenciaBanco(jTexAgenciaBanco.getText());
            cli.setNumContaBanco(jTexNumContaBanco.getText());
            cli.setCdPortador(jTexCdPortador.getText());
            cli.setCdTipoPagamento(jTexCdTipoPagamento.getText());
            cli.setCdCondPagamento(jTexCdCondPag.getText());
            cli.setNomeContato(jTexNomeContato.getText().toUpperCase());
            cli.setTelefone(jForTelefone.getText());
            cli.setCelular(jForCelular.getText());
            cli.setEmail(jTexEmail.getText().trim());
            cli.setUsuarioCadastro(su.getUsuarioConectado());
            dat.setData(data);
            data = dat.getData();
            cli.setDataCadastro(data);
            cli.setSituacao(jComSituacao.getSelectedItem().toString().substring(0, 1));
            ClientesDAO clidao = null;
            sql = "SELECT * FROM GCVCLIENTES WHERE CPF_CNPJ = '" + cpfCnpj
                    + "'";
            try {
                clidao = new ClientesDAO(conexao);
                if ("N".equals(oper)) {
                    clidao.adicionar(cli);
                } else {
                    clidao.atualizar(cli);
                }
            } catch (SQLException ex) {
                Logger.getLogger(ManterClientesOld.class.getName()).log(Level.SEVERE, null, ex);
            }
            limparTela();
            bloquearCampos();
            pesquisar();
    ...
    private void jButExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButExcluirActionPerformed
        ...
        if (!jForCdCpfCnpj.getText().isEmpty()) {
            try {
                ...
                ClientesDAO ccDAO = new ClientesDAO(conexao);
                ccDAO.excluir(cc);
                limparTela();
                jButPesquisarActionPerformed(evt);
            } catch (SQLException ex) {
    ...
    private void jTexSiglaUFCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_jTexSiglaUFCaretUpdate
        // TODO add your handling code here:
        if ("BA".equals(jTexSiglaUF.getText())) {
            jComOptanteSimbahia.setEnabled(true);
        }
        if ("AM".equals(jTexSiglaUF.getText())) {
            jComOptanteSuframa.setEnabled(true);
            jTexCdSuframa.setEnabled(true);
        }
    }//GEN-LAST:event_jTexSiglaUFCaretUpdate
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ManterClientesOld.class
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ManterClientesOld(su, conexao).setVisible(true);
            }
        });
    }
