 ...
 */
public class ManterItemTipoVerniz extends javax.swing.JDialog {

    // Variáveis de intância de objetos principal da classe
    private ItemTipoVerniz regCorr;
    private List< ItemTipoVerniz> resultado;
    private ItemTipoVernizDAO itvdao;
    private CItemTipoVerniz pitv;

    // Variáveis de instância de objetos de registros correlatos
    private static TipoVerniz motvz;
    private static ItemTipoVerniz moditv;
     ...
     */
    public ManterItemTipoVerniz(java.awt.Frame parent, boolean modal, TipoVerniz modtvz, SessaoUsuario su, Connection conexao) {
        super(parent, modal);
        this.motvz = modtvz;
        this.su = su;
        this.conexao = conexao;
        this.cdTipoVerniz = modtvz.getCdTipoVerniz();
        formato = NumberFormat.getInstance();
        formato.setMinimumFractionDigits(2);
        pitv = new CItemTipoVerniz(conexao);
        moditv = new ItemTipoVerniz();
        try {
            itvdao = new ItemTipoVernizDAO(conexao);
        } catch (SQLException ex) {
            Logger.getLogger(ManterItemTipoVerniz.class.getName()).log(Level.SEVERE, null, ex);
        }

        sql = "SELECT * FROM GCVITEMTIPOVERNIZ WHERE CD_TIPOVERNIZ = '" + cdTipoVerniz + "'";
        oper = "N";
        initComponents();
    ...
    private void upRegistros() {
        ...
        pitv.mostrarPesquisa(moditv, idxCorr - 1);
        DataSistema dat = new DataSistema();
        jTexCdMaterial.setText(moditv.getCdMaterial().trim());
        jTexNomeMaterial.setText(moditv.getNomeMaterial().trim());
        jTexCdUnidMedida.setText(moditv.getCdUnidMedida());
        jTexNomeUnidMedida.setText(moditv.getNomeUnidMedida());
        jForQtde.setText(String.valueOf(moditv.getQtde()));
        oldValorUnit = moditv.getQtde();
        jComSituacao.setSelectedIndex(Integer.parseInt(moditv.getSituacao()));
        oldSituacao = moditv.getSituacao();
        jTexCadastradoPor.setText(moditv.getUsuarioCadastro());
        jForDataCadastro.setText(dat.getDataConv(Date.valueOf(moditv.getDataCadastro())));
        changeValorUnit = false;
        if (moditv.getDataModificacao() != null) {
            jForDataModificacao.setText(dat.getDataConv(Date.valueOf(moditv.getDataModificacao())));
        }
    ...
    private void salvarRegistro() {
        if (jTexCdMaterial.getText().isEmpty() || jTexCdUnidMedida.getText().isEmpty()
                || jForQtde.getText().isEmpty() || jComSituacao.getSelectedIndex() == 0) {
            JOptionPane.showMessageDialog(null, "Os campos Material, Unid. Med. Qtde. e Situação são obrigatórios.");
        } else {
            DataSistema dat = new DataSistema();
            ItemTipoVerniz itv = new ItemTipoVerniz();
            String data = null;
            itv.setCdTipoVerniz(cdTipoVerniz);
            itv.setCdMaterial(jTexCdMaterial.getText().trim());
            itv.setCdUnidMedida(jTexCdUnidMedida.getText());
            try {
                itv.setQtde(formato.parse(jForQtde.getText()).doubleValue());
            } catch (ParseException ex) {
                Logger.getLogger(ManterTabela.class.getName()).log(Level.SEVERE, null, ex);
            }
            dat.setData(data);
            data = dat.getData();
            itv.setDataCadastro(data);
            itv.setUsuarioCadastro(su.getUsuarioConectado());
            itv.setSituacao(jComSituacao.getSelectedItem().toString().substring(0, 1));
            ItemTipoVernizDAO itvdao = null;
            CItemTipoVerniz citv = new CItemTipoVerniz(conexao);
            try {
                itvdao = new ItemTipoVernizDAO(conexao);
                if ("N".equals(oper)) {
                    sql = "SELECT * FROM GCVITEMTIPOVERNIZ WHERE CD_TIPOVERNIZ = '" + itv.getCdTipoVerniz().trim() + "'";
                    itvdao.adicionar(itv);
                    motvz.setAtualizacao(true);
                } else {
                    sql = "SELECT * FROM GCVITEMTIPOVERNIZ WHERE CD_TIPOVERNIZ = '" + moditv.getCdTipoVerniz() + "'";
                    itv.setUsuarioModificacao(su.getUsuarioConectado());
                    itv.setDataModificacao(data);
                    itvdao.atualizar(itv);
                    if (oldValorUnit != itv.getQtde()) {
                        motvz.setUsuarioModificacao(su.getUsuarioConectado());
                        motvz.setDataModificacao(data);
                        motvz.setAtualizacao(true);
                    }
    ...
    private void zoomMaterial() {
        ...
        jTexCdUnidMedida.setText(zoom.cdUnidMedida().trim());
    }
    ...
    private void initComponents() {
        ...
        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setTitle("Manter Item Tipo Verniz");
        setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
        setModal(true);

        jPanItemTipoVerniz.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createEtchedBorder(), "Incluir Item", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 11))); // NOI18N
        ...
        jForQtde.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        jForQtde.setEnabled(false);
        ...
        javax.swing.GroupLayout jPanItemTipoVernizLayout = new javax.swing.GroupLayout(jPanItemTipoVerniz);
        jPanItemTipoVerniz.setLayout(jPanItemTipoVernizLayout);
        jPanItemTipoVernizLayout.setHorizontalGroup(
            jPanItemTipoVernizLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanItemTipoVernizLayout.createSequentialGroup()
                .addGap(49, 49, 49)
                .addGroup(jPanItemTipoVernizLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addComponent(jLabQtde)
                    .addComponent(jLabCdUnidMedida)
                    .addComponent(jLabCdMaterial))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(jPanItemTipoVernizLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanItemTipoVernizLayout.createSequentialGroup()
        ...
        jPanItemTipoVernizLayout.setVerticalGroup(
            jPanItemTipoVernizLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanItemTipoVernizLayout.createSequentialGroup()
                .addContainerGap()
                .addGroup(jPanItemTipoVernizLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jLabCdMaterial)
        ...
        jLabReg.setText("\\");
            ...
            layout.setHorizontalGroup(
                ...
                .addComponent(jPanItemTipoVerniz, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
    ...
    private void jButExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButExcluirActionPerformed
        ...
        if (!jTexCdMaterial.getText().isEmpty()) {
            try {
                DataSistema dat = new DataSistema();
                String data = null;
                ItemTipoVerniz ea = new ItemTipoVerniz();
                ea.setCdTipoVerniz(cdTipoVerniz);
                ea.setCdMaterial(jTexCdMaterial.getText().toUpperCase());
                ItemTipoVernizDAO eaDAO = new ItemTipoVernizDAO(conexao);
                eaDAO.excluir(ea);
                dat.setData(data);
                data = dat.getData();
                motvz.setDataModificacao(data);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ManterItemTipoVerniz.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ManterItemTipoVerniz dialog = new ManterItemTipoVerniz(new javax.swing.JFrame(),
                        true, motvz, su, conexao);
    ...
    private javax.swing.JPanel jPanItemTipoVerniz;
