 ...
 */
public class CondicaoPagamentoDAO extends AbstractTableModel {
    ...
    public CondicaoPagamentoDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(CondicaoPagamento cp) {
        String sql = "INSERT INTO GFCCONDICAOPAGAMENTO(CD_CONDPAG, NOME_CONDPAG, NUM_PARCELAS,"
                + "USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO)"
                + " VALUES(?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cp.getCdCondpag());
            pstmt.setString(2, cp.getNomeCondPag());
            pstmt.setInt(3, cp.getNumParcelas());
            pstmt.setString(4, cp.getUsuarioCadastro());
            pstmt.setString(5, cp.getDataCadastro());
            pstmt.setString(6, cp.getDataModificacao());
            pstmt.setString(7, cp.getSituacao());
            pstmt.execute();
            pstmt.close();
            ...
            if (ex.getMessage().contains("Duplicate")) {
                ...
                JOptionPane.showMessageDialog(null, "Erro na gravação da Condição de Pagamento, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(CondicaoPagamento cp){
        String sql = "UPDATE GFCCONDICAOPAGAMENTO SET NOME_CONDPAG = ?,"
                + "NUM_PARCELAS = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + " WHERE CD_CONDPAG = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cp.getNomeCondPag());
            pstmt.setInt(2, cp.getNumParcelas());
            pstmt.setString(3, cp.getDataModificacao());
            pstmt.setString(4, cp.getSituacao());
            pstmt.setString(5, cp.getCdCondpag());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(CondicaoPagamento cp){
        String sql = "DELETE FROM GFCCONDICAOPAGAMENTO WHERE CD_CONDPAG = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cp.getCdCondpag());
            pstmt.execute();
    ...
    public void selecionar(List<CondicaoPagamento> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new CondicaoPagamento(
                                rs.getString("cd_condpag"),
                                rs.getString("nome_condpag"),
