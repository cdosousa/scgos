 ...
 * @version 0.01beta_0917 created on 01/11/2017
 */
public class BancosDAO extends AbstractTableModel {
    ...
    public BancosDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(Bancos bc) {
        String sql = "INSERT INTO GFCBANCOS(CD_BANCO, NOME_BANCO, TIPO_LOGRADOURO, LOGRADOURO, NUMERO,"
                + " COMPLEMENTO, BAIRRO, CD_MUNICIPIO_IBGE, CD_UF_SIGLA, CEP, CONTATO, TELEFONE, CELULAR, EMAIL"
                + ", USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO)"
                + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, bc.getCdBanco());
            pstmt.setString(2, bc.getNomeBanco());
            pstmt.setString(3, bc.getTipoLogradouro());
            pstmt.setString(4, bc.getLogradouro());
            pstmt.setString(5, bc.getNumero());
            pstmt.setString(6, bc.getComplemento());
            pstmt.setString(7, bc.getBairro());
            pstmt.setString(8, bc.getCdMunicipioIbge());
            pstmt.setString(9, bc.getCdUfSigla());
            pstmt.setString(10, bc.getCep());
            pstmt.setString(11, bc.getContato());
            pstmt.setString(12, bc.getTelefone());
            pstmt.setString(13, bc.getCelular());
            pstmt.setString(14, bc.getEmail());
            pstmt.setString(15, bc.getUsuarioCadastro());
            pstmt.setString(16, bc.getDataCadastro());
            pstmt.setString(17, bc.getDataModificacao());
            pstmt.setString(18, bc.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código do banco já cadastrado");
            } else {
    ...
    public void atualizar(Bancos bc){
        String sql = "UPDATE GFCBANCOS SET NOME_BANCO = ?,"
                + "TIPO_LOGRADOURO = ?,"
                + "LOGRADOURO = ?,"
                + "NUMERO = ?,"
                + "COMPLEMENTO = ?,"
        ...
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, bc.getNomeBanco());
            pstmt.setString(2, bc.getTipoLogradouro());
            pstmt.setString(3, bc.getLogradouro());
            pstmt.setString(4, bc.getNumero());
            pstmt.setString(5, bc.getComplemento());
            pstmt.setString(6, bc.getBairro());
            pstmt.setString(7, bc.getCdMunicipioIbge());
            pstmt.setString(8, bc.getCdUfSigla());
            pstmt.setString(9, bc.getCep());
            pstmt.setString(10, bc.getContato());
            pstmt.setString(11, bc.getTelefone());
            pstmt.setString(12, bc.getCelular());
            pstmt.setString(13, bc.getEmail());
            pstmt.setString(14, bc.getDataModificacao());
            pstmt.setString(15, bc.getSituacao());
            pstmt.setString(16, bc.getCdBanco());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(Bancos bc){
        String sql = "DELETE FROM GFCBANCOS WHERE CD_BANCO = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, bc.getCdBanco());
            pstmt.execute();
    ...
    public void selecionar(List<Bancos> resultado, String sql) {
        try {
            if (!statusConexao) {
