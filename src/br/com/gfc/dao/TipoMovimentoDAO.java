 ...
 */
public class TipoMovimentoDAO extends AbstractTableModel {
    ...
    public TipoMovimentoDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(TipoMovimento tm) {
        String sql = "INSERT INTO GFCTIPOMOVIMENTO(CD_TIPOMOVIMENTO, NOME_TIPOMOVIMENTO, CD_CONTAREDUZIDA,"
                + "SIT_LANCAMENTO, SIT_CONTRAPARTIDA, USUARIO_CADASTRO, DATA_CADASTRO, HORA_CADASTRO, USUARIO_MODIFICACAO,"
                + "DATA_MODIFICACAO,HORA_MODIFICACAO, SITUACAO)"
                + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, tm.getCdTipoMovimento());
            pstmt.setString(2, tm.getNomeTipoMovimento());
            pstmt.setString(3, tm.getCdContaReduzida());
            pstmt.setString(4, tm.getSituacaoLancamento());
            pstmt.setString(5, tm.getSituacaoContraPartida());
            pstmt.setString(6, tm.getUsuarioCadastro());
            pstmt.setString(7, tm.getDataCadastro());
            pstmt.setString(8, tm.getHoraCadastro());
            pstmt.setString(9, tm.getUsuarioModificacao());
            pstmt.setString(10, tm.getDataModificacao());
            pstmt.setString(11, tm.getHoraModificacao());
            pstmt.setString(12, tm.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Tipo de Movimento já cadastrado");
            } else {
    ...
    public void atualizar(TipoMovimento tm){
        String sql = "UPDATE GFCTIPOMOVIMENTO SET NOME_TIPOMOVIMENTO = ?,"
                + "CD_CONTAREDUZIDA = ?,"
                + "SIT_LANCAMENTO = ?,"
                + "SIT_CONTRAPARTIDA = ?,"
                + "USUARIO_MODIFICACAO = ?,"
                ...
                + "HORA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_TIPOMOVIMENTO = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, tm.getNomeTipoMovimento());
            pstmt.setString(2, tm.getCdContaReduzida());
            pstmt.setString(3, tm.getSituacaoLancamento());
            pstmt.setString(4, tm.getSituacaoContraPartida());
            pstmt.setString(5, tm.getUsuarioModificacao());
            pstmt.setString(6, tm.getDataModificacao());
            pstmt.setString(7, tm.getHoraModificacao());
            pstmt.setString(8, tm.getSituacao());
            pstmt.setString(9, tm.getCdTipoMovimento());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(TipoMovimento tm){
        String sql = "DELETE FROM GFCTIPOMOVIMENTO WHERE CD_TIPOMOVIMENTO = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, tm.getCdTipoMovimento());
            pstmt.execute();
    ...
    public void selecionar(List<TipoMovimento> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new TipoMovimento(
                                rs.getString("cd_tipomovimento"),
                                rs.getString("nome_tipomovimento"),
                                rs.getString("cd_contareduzida"),
                                rs.getString("sit_lancamento"),
                                rs.getString("sit_contrapartida"),
