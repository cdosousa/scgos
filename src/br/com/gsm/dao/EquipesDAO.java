 ...
 */
public class EquipesDAO extends AbstractTableModel {
    ...
    public EquipesDAO() throws SQLException {
    ...
    public void adicionar(Equipes eqp) {
        String sql = "INSERT INTO GSMEQUIPE(CD_EQUIPE, NOME_EQUIPE, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, eqp.getCdEquipe());
            pstmt.setString(2, eqp.getNomeEquipe());
            pstmt.setString(3, eqp.getUsuarioCadastro());
            pstmt.setString(4, eqp.getDataCadastro());
            pstmt.setString(5, eqp.getDataModificacao());
            pstmt.setString(6, String.valueOf(eqp.getSituacao()));
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Equipe já cadastrado");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação da Equipe, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }

    // método para poder atualizar o registro
    public void atualizar(Equipes eqp) {
        String sql = "UPDATE GSMEQUIPE SET NOME_EQUIPE = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_EQUIPE = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, eqp.getNomeEquipe());
            pstmt.setString(2, eqp.getDataModificacao());
            pstmt.setString(3, String.valueOf(eqp.getSituacao()));
            pstmt.setString(4, eqp.getCdEquipe());
            pstmt.execute();
            pstmt.close();
            if (!eqp.isAtualizacao()) {
    ...
    public void selecionar(List<Equipes> resultado, String sql) {
        try {
            if (!statusConexao) {
    ...
    public void excluir(Equipes eqp) {
        String sql = "DELETE FROM GSMEQUIPE WHERE CD_EQUIPE = ?";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, eqp.getCdEquipe());
            pstmt.execute();
