 ...
 */
public class TarefasDAO extends AbstractTableModel {
    ...
    public TarefasDAO() throws SQLException {
    ...
    public void adicionar(Tarefas ta) {
        String sql = "INSERT INTO GSMTAREFAS(CD_TAREFA, NOME_TAREFA, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ta.getCdTarefa());
            pstmt.setString(2, ta.getNomeTarefa());
            pstmt.setString(3, ta.getUsuarioCadastro());
            pstmt.setString(4, ta.getDataCadastro());
            pstmt.setString(5, ta.getDataModificacao());
            pstmt.setString(6, ta.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de tarefa já cadastrado");
            } else {
    ...
    public void atualizar(Tarefas ta){
        String sql = "UPDATE GSMTAREFAS SET NOME_TAREFA = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_TAREFA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ta.getNomeTarefa());
            pstmt.setString(2, ta.getDataModificacao());
            pstmt.setString(3, ta.getSituacao());
            pstmt.setString(4, ta.getCdTarefa());
            pstmt.execute();
            pstmt.close();
    ...
    public void excluir(Tarefas ta){
        String sql = "DELETE FROM GSMTAREFAS WHERE CD_TAREFA = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ta.getCdTarefa());
            pstmt.execute();
    ...
    public void selecionar(List<Tarefas> resultado, String sql) {
        try {
            if (!statusConexao) {
