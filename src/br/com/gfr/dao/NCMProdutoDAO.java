 ...
 */
public class NCMProdutoDAO extends AbstractTableModel {
    ...
    public NCMProdutoDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(NCMProduto ncm) {
        String sql = "INSERT INTO GFRNCM(CD_NCM, NOME_NCM, CD_ORIGEM_CSTA, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ncm.getCdNcm());
            pstmt.setString(2, ncm.getNomeNcm());
            pstmt.setString(3, ncm.getCdOrigemCsta());
            pstmt.setString(4, ncm.getUsuarioCadastro());
            pstmt.setString(5, ncm.getDataCadastro());
            pstmt.setString(6, ncm.getDataModificacao());
            pstmt.setString(7, ncm.getSituacao());
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de NCM já cadastrada");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação do NCM, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(NCMProduto ncm){
        String sql = "UPDATE GFRNCM SET NOME_NCM = ?,"
                + "CD_ORIGEM_CSTA = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_NCM = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, ncm.getNomeNcm());
            pstmt.setString(2, ncm.getCdOrigemCsta());
            pstmt.setString(3, ncm.getDataModificacao());
            pstmt.setString(4, ncm.getSituacao());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<NCMProduto> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new NCMProduto(
                                rs.getString("cd_ncm"),
                                rs.getString("nome_ncm"),
