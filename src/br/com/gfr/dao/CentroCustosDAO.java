 ...
 */
public class CentroCustosDAO extends AbstractTableModel {
    ...
    public CentroCustosDAO(Connection conexao) throws SQLException {
    ...
    public void adicionar(CentroCustos cc) {
        String sql = "INSERT INTO GFRCCUSTO(CD_CCUSTO, NOME_CCUSTO, PERC_RATEIO, USUARIO_CADASTRO, DATA_CADASTRO, DATA_MODIFICACAO, SITUACAO) VALUES(?,?,?,?,?,?,?)";
        try {
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getCdCcusto());
            pstmt.setString(2, cc.getNomeCcusto());
            pstmt.setFloat(3, cc.getPercRateio());
            pstmt.setString(4, cc.getUsuarioCadastro());
            pstmt.setString(5, cc.getDataCadastro());
            pstmt.setString(6, cc.getDataModificacao());
            pstmt.setString(7, (cc.getSituacao()));
            pstmt.execute();
            pstmt.close();
            JOptionPane.showMessageDialog(null, "Registro gravado com sucesso!");
        } catch (SQLException ex) {
            if (ex.getMessage().contains("Duplicate")) {
                JOptionPane.showMessageDialog(null, "Código de Centro de Custo já cadastrada");
            } else {
                JOptionPane.showMessageDialog(null, "Erro na gravação do Centro de Custo, informe o administrador do sistema!\nErr: " + ex);
            }
        }
    }
    
    // método para poder atualizar o registro
    public void atualizar(CentroCustos cc){
        String sql = "UPDATE GFRCCUSTO SET NOME_CCUSTO = ?,"
                + "PERC_RATEIO = ?,"
                + "DATA_MODIFICACAO = ?,"
                + "SITUACAO = ?"
                + "WHERE CD_CCUSTO = ?";
        try{
            PreparedStatement pstmt = conexao.prepareStatement(sql);
            pstmt.setString(1, cc.getNomeCcusto());
            pstmt.setFloat(2, cc.getPercRateio());
            pstmt.setString(3, cc.getDataModificacao());
            pstmt.setString(4, cc.getSituacao());
            pstmt.execute();
            pstmt.close();
    ...
    public void selecionar(List<CentroCustos> resultado, String sql) {
        try {
            if (!statusConexao) {
                ...
                while (rs.next()) {
                    try {
                        resultado.add(new CentroCustos(
                                rs.getString("cd_ccusto"),
                                rs.getString("nome_ccusto"),
                                rs.getFloat("perc_rateio"),
